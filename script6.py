...
for bash in range(0, nn):
    with open ('run' + str(bash) + '.sh', 'w') as f:
        f.write('#!/bin/bash' + '\n')
        f.write('path="/etc/network/interfaces"' + '\n')
        f.write('nbr=' + str(nbr) + '\n')
        f.write('function deploy_bridges {')
        f.write('for (( br=101; br <= $((100 + ' + str(L[bash][1]) + ')); br++ ))' + '\n')
        f.write('do' + '\n')
        f.write('echo >> $path' + '\n')
        ...
        for machines in vmtp:
            if machines[1] != 'internet':
                #for i in
                f.write('#создание машины ' + str(machines[0]) + '\n')   
                f.write('function deploy_stand {' + '\n')
                f.write('br=vmbr$((100 + $nbr * $i + 1))' + '\n')
                f.write('nvm=$((500 + 10 * $i)) ' + '\n')
                f.write('qm create $nvm --name' + str(machines[0]) + '--pool "pool-g1-node1-$i" --cores 2 --memory 3072 --ostype l26 --scsihw virtio-scsi-single --net0 e1000,bridge=$br,firewall=1 --vga qxl' + '\n')
                f.write('if [ $? -ne 0 ]; then echo "Ошибка создания HQ-SRV для стенда №$i"; exit 2; fi' + '\n')
                f.write('sleep 1' + '\n')
                f.write('qm importdisk $nvm /mnt/pve/BACKUP/qcow2/DE2024/SRV.qcow2 STORAGE --format qcow2' + '\n')
                f.write('if [ $? -ne 0 ]; then echo "Ошибка импорта диска для HQ-SRV на стенде №$i"; exit 3; fi' + '\n')
                f.write('sleep 1' + '\n')
                f.write('qm set $nvm -ide0 STORAGE:$nvm/vm-$nvm-disk-0.qcow2 --boot order=ide0' + '\n')
                f.write('echo "$nvm" >> .temp_stand_416_g1_$i # вносим информацию о ВМ в файл' + '\n')
                f.write('echo; echo "ВМ $vm с номером $nvm на стенде №$i развернута"; echo' + '\n')
                f.write('' + '\n')
